...
describe("keyword-detector agent-specific ultrawork messages", () => {
  ...
  test("should skip ultrawork injection when agent is plan", async () => {
    // given - collector and plan agent
    const collector = new ContextCollector()
    const hook = createKeywordDetectorHook(createMockPluginInput(), collector)
    ...
    // when - ultrawork keyword detected with plan agent
  ...
  test("should skip ultrawork for plan but inject for sisyphus", async () => {
    // given - two sessions, one with plan, one with sisyphus
    const collector = new ContextCollector()
    const hook = createKeywordDetectorHook(createMockPluginInput(), collector)

    // First session with plan
    const planSessionID = "plan-first"
    const planOutput = {
      message: {} as Record<string, unknown>,
      parts: [{ type: "text", text: "ultrawork plan" }],
    }
    await hook["chat.message"]({ sessionID: planSessionID, agent: "plan" }, planOutput)
    ...
    // then - plan should have no injection, sisyphus should have normal ultrawork
    const planTextPart = planOutput.parts.find(p => p.type === "text")
    expect(planTextPart!.text).toBe("ultrawork plan")
  ...
  test("should use session state agent over stale input.agent (bug fix)", async () => {
    // given - same session, agent switched from plan to sisyphus in session state
    const collector = new ContextCollector()
    const hook = createKeywordDetectorHook(createMockPluginInput(), collector)
    ...
    // when - hook receives stale input.agent="plan" but session state says "Sisyphus"
    await hook["chat.message"]({ sessionID, agent: "plan" }, output)

    // then - should use Sisyphus from session state, NOT plan from stale input
    const textPart = output.parts.find(p => p.type === "text")
    expect(textPart).toBeDefined()
    expect(textPart!.text).toContain("YOU MUST LEVERAGE ALL AVAILABLE AGENTS")
    expect(textPart!.text).not.toContain("YOU ARE A PLANNER, NOT AN IMPLEMENTER")
  ...
  test("should fall back to input.agent when session state is empty and skip ultrawork for plan", async () => {
    ...
    // when - hook receives input.agent="plan" with no session state
    await hook["chat.message"]({ sessionID, agent: "plan" }, output)

    // then - plan fallback from input.agent, ultrawork skipped
    const textPart = output.parts.find(p => p.type === "text")
    expect(textPart).toBeDefined()
